	...
	<table class="table table-bordered table-hover">
        ...
        foreach ($reference as $d):
            ...
            if ($d['Lesson']['is_block'] === '0'){
                echo $this->Html->link(__('Block'),array('controller' => 'Admin','action' => 'blockLesson',$d['Lesson']['coma_id']),array('link_type' => 'block_link'));
            }
            else{
                echo $this->Html->link(__('Unblock'),array('controller' => 'Admin','action' => 'unBlockLesson',$d['Lesson']['coma_id']),array('link_type' => 'un_block_link'));
            }
            echo '</td>';
            echo '<td>';
            echo $this->Html->link(__('Delete'),array('controller' => 'Admin','action' => 'deleteLesson',$d['Lesson']['coma_id']),array('link_type' => 'delete_link'));
            echo '</td></tr>';
        endforeach;
        ?>
	</table> 
                ...
                function (data){                
                    if (data.trim() === '1'){
                        ...
                        if (type === 'delete_link'){                             
                            ...
                            link.attr('href',<?php echo "'".$this->Html->url(array('controller' => 'Admin','action' => 'unBlockLesson',$d['Lesson']['coma_id']))."'"; ?>);
                            link.attr('link_type','un_block_link');                           
                        }
                        else{
                            link.html(<?php echo "'".__('Block')."'"; ?>);
                            link.attr('href',<?php echo "'".$this->Html->url(array('controller' => 'Admin','action' => 'blockLesson',$d['Lesson']['coma_id']))."'"; ?>);
