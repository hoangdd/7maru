...
<div class="row col-md-12">
	<table class="table table-bordered table-hover">
		...
		foreach ($reference as $d):                
			echo '<tr class="linktr data-row">'.'<td>'.$this->Html->link($d['Data']['file_name'],array('controller' => 'lesson','action' => 'viewContent',$d['Data']['file_id'])).'</td>'.'<td>'.$d['Data']['type'].'</td>'.'<td>'.$d['Lesson']['name'].'</td><td>'.$d['Lesson']['Author']['username'].'</td><td>'.$d['Lesson']['created'].'</td>';

			//search data @hoangdd
			echo '<td style ="display:none"><div class="search-data">'
					.$d['Data']['file_name'].'</div><div class="search-data">'
					.$d['Data']['type'].'</div><div class="search-data">'
					.$d['Lesson']['name'].'</div><div class="search-data">'
					.$d['Lesson']['Author']['username'].'</div></td>';
			//
			...
			if( keyword.length == 0){
				...
				$('.data-row').each(function(){
					...
					$(this).find('.search-data').each(function(){
						str = $(this).text();
						if( f == false && str.toLowerCase().search(keyword.toLowerCase()) >= 0 ){
							f = true;
						}
					});
					if( f ){
